...
db_Table_Client::db_Table_Client(const bool& statusCreateDB, mySQL_db& connect, char mode) 
	: db_Table(statusCreateDB, connect), _mode(mode)
{
	if (_statusCreateDB == true)
	{
		...
		else if(existence_table_clients == false && _mode == 's')
		{
		...
		if(existence_table_clients == false && _mode == 'c')
			this->_createTable_Clients = false;
	}
}
...
db_Table_Messages::db_Table_Messages(const bool& statusCreateDB, mySQL_db& connect, char mode) 
	: db_Table(statusCreateDB, connect), _mode(mode)
{
	if (_statusCreateDB == true)
	{
		...
		else if(existence_table_1 == false && _mode == 's')
		{
		...
		if(existence_table_1 == false && _mode == 'c')
			this->_createTable_Messages = false;
	}
}
...
db_Table_hPassword::db_Table_hPassword(const bool& statusCreateDB, mySQL_db& connect, char mode) 
	: db_Table(statusCreateDB, connect), _mode(mode)
{
	/*---password_hash---*/
	...
	else if(existence_table_hp == false && _mode == 's')
	{
	...
	if(existence_table_hp == false && _mode == 'c')
		this->_createTable_hp = false;
}
